...
class PRService:
    ...
    async def create(self, task_id: str, data: PRCreate) -> PR:
        ...
        If body is not provided, this method will load the pull_request_template
        (if available) and generate a description using LLM.
        ...
        pr_body = data.body
        if not pr_body:
            # Load template and generate body using LLM
            template = await self._load_pr_template(repo_obj)

            # Get diff for description generation
            diff = ""
            if run.worktree_path:
            ...
            if diff:
                pr_body = await self._generate_description_for_new_pr(
                    diff=diff,
                    template=template,
                    task=task,
                    title=data.title,
                ...
                # Fallback when no diff available
                pr_body = f"Generated by dursor\n\n{run.summary or ''}"
