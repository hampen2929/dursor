...
class PRService:
    ...
    def __init__(
        ...
        template_service: PRTemplateService | None = None,
    ):
        self.pr_dao = pr_dao
        ...
        self.template_service = template_service or PRTemplateService()
    ...
    async def create(self, task_id: str, data: PRCreate) -> PR:
        ...
        # Create PR body using dursor block approach
        template = await self._load_pr_template(repo_obj, data.template_path)
        description_src = (data.body or run.summary or "").strip()
        if template:
            # Use non-destructive dursor block composition
            pr_body = self.template_service.compose_pr_body(
                template=template,
                generated_content=description_src,
                title=data.title,
            )
        else:
...
"""
    ...
    async def _generate_description_for_new_pr(
        ...
        Uses the dursor block approach:
        1. LLM generates only the content for the dursor block
        2. Template service composes the final body with template preservation
        ...
        """
        prompt = self._build_description_prompt_for_new_pr(diff, task, title, run)
        ...
        try:
            ...
            generated_content = await llm_client.generate(
                ...
                system=(
                    ...
                    "and concise PR descriptions. Output only the requested content."
                ),
            )

            # Use dursor block composition with template
            if template:
                return self.template_service.compose_pr_body(
                    template=template,
    ...
    def _build_description_prompt_for_new_pr(
        ...
        This prompt generates content for the dursor block only.
        The template composition is handled separately by PRTemplateService.
        ...
        prompt_parts = [
            "Generate a concise PR description based on the following information.",
            "Output ONLY the content in the specified format, nothing else.",
            "",
            "## Task Description",
            task.title or "(None)",
            "",
            ...
            "",
            "## Output Format",
            "Generate ONLY the following sections (no additional text):",
            "",
            "## Summary",
            "(1-3 sentences describing what this PR does and why)",
            "",
            "## Changes",
            "(Bullet points listing the main changes)",
            "",
            "## Test Plan",
    ...
    def _generate_fallback_description_for_new_pr(
        ...
        Uses the dursor block approach for template composition.
        ...
        # Generate the dursor block content
        generated_content = f"""## Summary
{summary}
...
"""
    ...
    async def regenerate_description(self, task_id: str, pr_id: str) -> PR:
        ...
        Uses the dursor block approach to preserve user modifications:
        - If PR body has a dursor block, only updates that block
        - If no dursor block, adds one at the appropriate position
        - User edits outside the dursor block are preserved
        ...
        # Generate new content for dursor block using LLM
        new_generated_content = await self._generate_dursor_block_content(
            diff=cumulative_diff,
            task=task,
            pr=pr,
        )

        # Update the dursor block in the existing PR body
        # This preserves user modifications outside the dursor block
        existing_body = pr.body or ""
        new_description = self.template_service.update_dursor_block(
            existing_body=existing_body,
            new_generated_content=new_generated_content,
        )
    ...
    async def _generate_dursor_block_content(
        self,
        diff: str,
        task: Task,
        pr: PR,
    ) -> str:
        """Generate content for the dursor block during regeneration.
        ...
        Returns:
            Generated content for the dursor block.
        """
        prompt = self._build_dursor_block_prompt(diff, task, pr)
        ...
        try:
            ...
            return self._generate_fallback_dursor_content(diff, pr)

    def _build_dursor_block_prompt(self, diff: str, task: Task, pr: PR) -> str:
        """Build prompt for dursor block content generation.
        ...
        return f"""Generate a concise PR description based on the following information.
Output ONLY the content in the specified format, nothing else.
...
"""
    ...
    def _generate_fallback_dursor_content(self, diff: str, pr: PR) -> str:
        """Generate fallback content for dursor block.
...
"""
    ...
    async def _load_pr_template(self, repo: Repo, template_path: str | None = None) -> str | None:
        """Load repository's pull_request_template.

        Uses the PRTemplateService for template enumeration and selection.
        If template_path is provided, attempts to load that specific template.
        Otherwise, uses the default template selection logic.
        ...
        if template_path:
            ...
            template = self.template_service.get_template_by_path(workspace_path, template_path)
            return template.content if template else None

        # Use default template selection
        template = self.template_service.get_default_template(workspace_path)
        return template.content if template else None
